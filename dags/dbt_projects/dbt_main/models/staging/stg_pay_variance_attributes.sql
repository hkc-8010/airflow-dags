...
{% set column_names =  ['id', 'included_in_gross', 'finalised', 'retro'] %}

-- junk dimension for pay variance attributes
-- group to get unique set per file
with source_rows_base as (
    select coalesce(included_in_gross, '') || '|' || coalesce(finalised, '') || '|' ||  coalesce(retro, '') as id
    , included_in_gross
    , finalised 
    , retro
    , min(id) as event_id                            
