...
with cte_detail_base as (
    ...
    , left(coalesce(to_char(cc.compensation_start_date, 'YYYYMMDD'),'') || repeat(' ', 8), 8) as start_date
    , repeat('NV', 2) as pay_scale_type
    , repeat('NV', 2) as pay_scale_area
    , left('NV' || repeat(' ', 8), 8) as pay_scale_group
    , left('NV' || repeat(' ', 6), 6) as salary_pay_grade_level
    , c.colleague_id
    , f.event_time
    , c.deleted
    ...
    , left('obh011' || repeat(' ', 8), 8) as program_name                       -- input
    , left('Employee Contract Grade File' || repeat(' ', 40), 40) as file_description      -- input
    , '+' as sign1
    ...
    from cte_header
    union all
    select record_type_code || record_action || financial_institution_id || source_application_code || employee_local_code || filler || global_id || start_date || pay_scale_type || pay_scale_area || pay_scale_group || salary_pay_grade_level as extract_column
    from cte_detail
