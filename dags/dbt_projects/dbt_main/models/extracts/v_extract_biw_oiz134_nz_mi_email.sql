...
, coalesce(c.email_address,'') || '\t' as "E-mail address"                                      -- NB. Adding a TAB to mimic the behaviour of the original files.
from cte_final f
inner join {{ ref('dim_colleague') }} c on f.colleague_key = c.colleague_key
where coalesce(c.email_address, '') <> ''
